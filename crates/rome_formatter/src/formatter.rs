...
impl<Options> Formatter<Options> {
    ...
    pub fn format_preceding_comments<L>(
        &self,
        token: &SyntaxToken<L>,
        ...
        L: Language + 'static,
    {
        ...
        let (prev_token_kind, trailing_trivia) =
            if let Some(prev_override) = self.get_prev_token_override(token) {
                (Some(prev_override), None)
            } else if let Some(prev_token) = token.prev_token() {
                (
                    Some(prev_token.kind()),
                    Some(prev_token.trailing_trivia().pieces()),
                )
            } else {
                (None, None)
            };

        let pieces = trailing_trivia
            .into_iter()
            .flatten()
            .chain(token.leading_trivia().pieces());

        // Lines before the next comment or the token.
        let mut lines_before = 0u32;
        let mut comments: SmallVec<[Comment<L>; 3]> = SmallVec::new();

        for piece in pieces {
            if piece.is_newline() {
                lines_before += 1;
            } else if let Some(comment) = piece.as_comments() {
                let comment_type = L::get_comment_type(&comment);
                let comment = Comment {
                    piece: comment,
                    comment_type,
                    lines_before,
        ...
        self.format_comments(comments, lines_before, token)
    }

    fn format_comments<L>(
        comments: I,
        lines_before: u32,
        token: SyntaxToken<L>,
    ) -> FormatResult<FormatElement>
